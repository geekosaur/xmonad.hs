...
-- @@@ INCOMPLETE, only lays out current screen --- need info for other screens
-- @@@@@ see bug 526, space is reserved at the farthest edges not on each monitor
setWorkarea :: Rectangle -> Rectangle -> X ()
setWorkarea (Rectangle sx sy _ _) (Rectangle x y w h) = withDisplay $ \dpy -> do
    -- return ()
    a <- getAtom "_NET_WORKAREA"
    c <- getAtom "CARDINAL"
    r <- asks theRoot
    -- @@@@@ for now assume all docks on primary monitor @@@@@@@@@ I hope
    si <- withDisplay getCleanedScreenInfo
    io $ changeProperty32 dpy r a c propModeReplace ([fi x - fi sx, fi y - fi sy, fi w, fi h] ++ replicate ((length si - 1) * 4) 0)
