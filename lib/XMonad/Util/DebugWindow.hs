...
-- - _NET_WM_STATE
-- - WM_TRANSIENT_FOR
-- - do we want window group? (potentially nasty/expensive)
--
-----------------------------------------------------------------------------
  ...
  case w' of
    ...
    Just (WindowAttributes x y wid ht bw m o) -> do
      ...
      t <- catchX' (wrap `fmap` getEWMHTitle  "VISIBLE" w) $
           ...
           return ""
      h' <- getMachine w
      tf <- withDisplay $ \d -> io $ getTransientForHint d w
      ...
      return $ concat [lb
                      ...
                      ,if null r then "" else '/':r
                      ,if null cmd then "" else ' ':cmd 
                      ,rb
                      ,if null st then "" else ' ':aList st
                      ,maybe "" ((" transient for " ++) . pad 8 '0' . flip showHex "") tf
                      ]
...
-- | dump _NET_WM_STATE
getEWMHState   :: Window -> X [String]
getEWMHState w =  do
    wS <- getAtom "_NET_WM_STATE"
    p <- withDisplay $ \d -> io $ getWindowProperty32 d wS w
    case p of
      Nothing  -> return []
      Just cs' -> do
        ns <- withDisplay $ \d -> io $ getAtomNames d (map (toEnum . fromEnum) cs')
        return $ map stripState ns
 
-- | Remove EWMH window state prefix if present, or output literally if not
stripState :: String -> String
stripState ('_':'N':'E':'T':'_':'W':'M':'_':'S':'T':'A':'T':'E':'_':xs) = map toLower xs
stripState xs = xs

-- | get _NET_WM_WINDOW_TYPE (same tune different pitch)
getEWMHType   :: Window -> X [String]
getEWMHType w =  do
    wS <- getAtom "_NET_WM_WINDOW_TYPE"
    p <- withDisplay $ \d -> io $ getWindowProperty32 d wS w
    case p of
      Nothing  -> return []
      Just cs' -> do
        ns <- withDisplay $ \d -> io $ getAtomNames d (map (toEnum . fromEnum) cs')
        return $ map stripType ns
 
-- | Remove EWMH window type prefix if present, or output literally if not
stripType :: String -> String
stripType ('_':'N':'E':'T':'_':'W':'M':'_':'W':'I':'N':'D':'O':'W':'_':'T':'Y':'P':'E':'_':xs) = map toLower xs
stripType xs = xs

-- | Format a list either to empty string or parenthesized, comma separated elements
aList    :: [String] -> String
aList [] =  ""
aList xs =  go "(" xs
  where
...
getRole   :: Window -> X String
getRole w =  do
  wR <- getAtom "WM_WINDOW_ROLE"
  p <- withDisplay $ \d -> io $ getWindowProperty8 d wR w
  return $ case p of
    Nothing -> ""
    Just cs -> map (toEnum . fromEnum) cs
